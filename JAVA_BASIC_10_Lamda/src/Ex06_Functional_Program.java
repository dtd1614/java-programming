...
제목, 본문, 그림 등과 같이 "무엇"을 화면에 나타내야 할지를 고민하는 것이 [ 선언형 프로그래밍 ]이다.
...
public class Ex06_Functional_Program {
	public static void main(String[] args) {
		...
		//선언형 (what) >> 로직 추상화 >> Stream API 함수형 >> 표현 람다식 
		int result2 =0;
		//ArrayList 는 stream 구현하고 있어요 
		result2 = list.stream()
				  .filter(n -> n % 2 == 0 ) // stream 리턴
				  .mapToInt(Integer :: intValue) //값을 정수로 변화 Integer 형식의 요소를 int 형식으로 변환하여 새로운 IntStream
				  .sum(); //소비
		
		System.out.println("result2 : " + result2);
	
	 /*			
	여기서 mapToInt는 스트림의 각 요소에 함수를 적용하여 새로운 스트림을 생성하는 중간 연산자입니다. 
	Integer::intValue는 Integer 클래스의 intValue() 메소드를 참조하는 메서드 레퍼런스입니다. 
	이것은 Integer 객체를 int로 변환하는 방법을 제공합니다.
	따라서 mapToInt(Integer::intValue)는 Integer 형식의 요소를 int 형식으로 변환하여 새로운 IntStream을 생성합니다. 
	*/
	//이러한 방식은 가독성을 높이고, 상태를 변경하는 모든 지점들이 map안으로 추상화되어 직접 상태를 변경하지 않아도 된다. 
	//선언형 프로그래밍에 속하는 대표적인 프로그래밍 방식이 함수형 프로그래밍이다. 
	}
